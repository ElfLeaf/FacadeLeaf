...
public class MyChangeableAttributedString {
    /**
    ...
    /**
     * we need the line metrics primarily to get the maximum ascent for all
     * characters.
     */
    LineMetrics[]      metrics;
    ...
    /**
     * In typography, kerning refers to adjusting the space between characters,
     * especially by placing two characters closer together than normal. Kerning
     * makes certain combinations of letters, such as WA, MW, TA, and VA, look
     * better.
     */
    private int        kerning;

    /**
     * Given an attributed string and the graphics environment it lives in, pull
     * it apart into its components.
     * 
     * @param g2
     *            graphics
     * @param aString
     *            attributed String
     */
    protected MyChangeableAttributedString(Graphics2D g2, AttributedString aString, int kerning) {
        ...
        for (int i = iter.getBeginIndex(); i < iter.getEndIndex(); i++) {
            ...
            if (font != null) {
                g2.setFont(font); // needed for getFont, -and-
    ...
    /**
     * Draw all characters according to their computed positions, and a color
    ...
    /**
     * Given a background image (for size only), pick a random spot such that
     * the entire string can be displayed. This method implicitly assumes that
     * all resizing issues have been taken care of first. If you resize
     * afterwards, any type of clipping is possible.
     * 
     * @param background
     *            the image that will lie under the text
     * @param startingPoint
     *            the suggested starting point, or null if any point is
     *            acceptable.
     * @return a Point2D object indicating the initial starting point of the
     *         text
     * @throws com.octo.captcha.CaptchaException
     *             if the image size is too small, or the word too long, or the
     *             fonts too large.
     */
    Point2D moveToRandomSpot(final BufferedImage background, Point2D startingPoint) {
        int maxHeight = (int) getMaxHeight();

        // this padding is necessary due to flaws in this algorithm and how it
        // interacts
        // with java. we are getting the logical bounds of the character, not
        // the actual
        // bound of the character. So ascenders on rotated characters may extend
        // out of the
        // box vertically and horizontally (for rotated letters), which means
        // that we can
        // place the letter such that the final character, f, say, has its top
        // outside the image.
        // the TextLayout class should be investigated more later; it didn't
        // work well earlier.
        final int arbitraryHorizontalPadding = 10;
        final int arbitraryVerticalPadding = 0;
        ...
        if (startingPoint == null) {
            ...
            // no suggested starting point is given - any spot on the vertical
            // axis is ok
        ...
        if (maxX < 0 || maxY < 0) {
            ...
            if (maxX < 0 && maxY > 0) {
                ...
                // ok, the text slammed into the end of the image. let's try
                // half the kerning:
                ...
                if (maxX < 0) {
                    ...
                    maxX = background.getWidth() - getTotalWidth();
                    if (maxX < 0) {
                        // that didn't work either. let's try gradual steps of
                        // negative kerning.
            ...
            throw new CaptchaException("word is " + problem + " try to use less letters, smaller font" + " or bigger background: "
                    + " text bounds = " + this + " with fonts " + this.getFontListing() + " versus image width = " + background.getWidth()
                    + ", height = " + background.getHeight());
        }

        int newX;
        if (startingPoint == null) {
            // no suggested starting point - the string can start anywhere
            // horizontal if
    ...
    /**
     * Rearrange the string so that all characters are treated as if they are as
     * wide as the widest character in the same string.
     * 
     * @param kerning
    ...
    void useMonospacing(double kerning) {
        ...
        // for every glyph after the first, space it out so that they are
        // maxWidth characters apart
    ...
    void useMinimumSpacing(double kerning) {
        ...
        for (int i = 1; i < length(); i++) {
            bounds[i].setRect(bounds[i - 1].getX() + bounds[i - 1].getWidth() + kerning, bounds[i].getY(), bounds[i].getWidth(),
                    bounds[i].getHeight());
        }
    }

    /**
     * Gradually reduce spacing between letters until the total length is less
     * than the final image width. In many cases, this will guarantee collisions
     * between the letters.
     * 
     * @param maxReductionPct
     *            maximum percentage reduction
     * @return if positive, the highest X value that can be safely used for
     *         placement of box; if negative, there is no safe way to display
     *         the text without clipping the ends.
     */
    double reduceHorizontalSpacing(int imageWidth, double maxReductionPct) {
        ...
        for (pct = stepSize; pct < maxReductionPct && maxX < 0; pct += stepSize) {
            for (int i = 1; i < length(); i++) {
                bounds[i].setRect((1 - pct) * bounds[i].getX(), bounds[i].getY(), bounds[i].getWidth(), bounds[i].getHeight());
            }
    ...
    void moveTo(double newX, double newY) {
        ...
        for (int i = 1; i < length(); i++) {
            bounds[i].setRect(newX + bounds[i].getX(), newY, bounds[i].getWidth(), bounds[i].getHeight());
        }
    }
